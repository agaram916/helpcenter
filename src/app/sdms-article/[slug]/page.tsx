...
const Usersetup = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const query = `*[_type == "sdmsarticle" && slug.current == $slug][0] {
          title,
          body,
          mainImage,
          publishedAt,
      ...
      <div className="inner-page">
        <nav className="navbar">
         <ul className="list-unstyled row mb-0">
            ...
            <li><Link href="/sdms-article">SDMS Article</Link></li>
          </ul>
        </nav>
      </div>
            ...
            <div className="row my-5">
              <div className="col-6 text-left">
                {article.prevArticle ? (
                  <>
                    ...
                    <Link href={`/sdms-article/${article.prevArticle.slug.current}`}>{article.prevArticle.title}</Link>
                  </>
                ) : null}
              </div>
              <div className="col-6 text-right">
                {article.nextArticle ? (
                  <>
                    <h4>Next article</h4>
                    <Link href={`/sdms-article/${article.nextArticle.slug.current}`}>{article.nextArticle.title}</Link>
                  </>
                ) : null}
              </div>
            </div>
