...
const SdmstroubleArticle = () => {
  const [articles, setArticles] = useState<Article[]>([]);

  useEffect(() => {
    async function fetchData() {
      const query = `*[_type == "sdmsarticle"] | order(_createdAt asc) {
        title,
        section,
        subtitle,
        url,
    ...
    <div className='product'>
      ...
      <div className='troble-article-list'>
        <div className='container'>
             <h1 className='text-center'>Logilab SDMS Tutorial Articles</h1>
             <div className="row">
  {/* Left Column */}
            ...
            <div key={subtitle} className="mb-3">
              ...
              <ul className="artile-list text-left">
                {groupedArticles[section].articles[subtitle].map((article, i) => (
                  <li key={i}>
                    <Link href={`/sdms-article/${article.slug}`}>{article.title}</Link>
                  </li>
                ))}
              </ul>
            </div>
...
export default SdmstroubleArticle;
